...
export class ManifestService {
  ...
  receiveSpecimen(id: string, sid: string, receivedBy: string): Observable<Specimen> {
    return this.http.post<Specimen>(`${this.apiUrl}/${id}/specimens/${sid}/receive`, { receivedBy }).pipe(
      tap(() => this.reloadSelectedManifest(id))
    );
  }

  flagSpecimen(id: string, sid: string, receivedBy: string, notes: string): Observable<Specimen> {
    return this.http.post<Specimen>(`${this.apiUrl}/${id}/specimens/${sid}/flag`, { receivedBy, notes }).pipe(
      tap(() => this.reloadSelectedManifest(id))
    );
  }
